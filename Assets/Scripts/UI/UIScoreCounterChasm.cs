...
public class UIScoreCounterChasm : MonoBehaviour
{
	...
	void FixedUpdate() {
		...
		if (deathText == true) {
			...
			myText.text = "<size=30>Your score is:</size>\n"
				+ "<color=#0095FF>" + FormatTime(GameManagerChasm.timeElapsedWhileAlive) + "</color>"
				+ "<color=#aaaaaa><size=30>\nYour personal best this session is:</size>\n"
				+ FormatTime(GameManagerChasm.timeElapsedWhileAliveBest)
				+ "</color>\n";

				if (GameManagerChasm.gameManagerChasmObj.GetComponent<GameManagerChasm>().gameBuild == GameBuild.WebGL) {
					myText.text += "<color=#aaaaaa><size=25>Scores on Newgrounds.com</size></color>";
				} else if (GameManagerChasm.gameManagerChasmObj.GetComponent<GameManagerChasm>().gameBuild == GameBuild.VR_Android) {
					myText.text += "<color=#aaaaaa><size=25>Scores on Oculus home.</size></color>";
				}

		} else {
			myText.text = FormatTime(GameManagerChasm.timeElapsedWhileAlive);
		}
	}
}
